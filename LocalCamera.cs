...
[assembly: MelonInfo(typeof(LocalCamera.Main), "LocalCamera_Mod", "1.0.9", "Nirvash, Mod By BaconEgg")]
...
namespace LocalCamera
{
    public class Main : MelonMod
    {
        ...
        public override void OnApplicationStart()
        {
            ...
            parentTracking = MelonPreferences.CreateEntry("LocalCamera", "ParentToTracking", true, "Parent Camera to Tracking Space");
            ...
            ExpansionKitApi.GetExpandedMenu(ExpandedMenu.QuickMenu).AddSimpleButton("Local Camera", () => ToggleCamera());
            ExpansionKitApi.GetExpandedMenu(ExpandedMenu.QuickMenu).AddSimpleButton("Reset Local Camera", () => ResetCamera());
            ExpansionKitApi.GetExpandedMenu(ExpandedMenu.QuickMenu).AddSimpleButton("Local Camera Menu", () => ToggleCamMenu());
        }
        ...
        public void ToggleCamMenu()
        {
            ...
            localcamMenu.AddSimpleButton("關閉菜單", () => localcamMenu.Hide());
            localcamMenu.AddSimpleButton("Local Camera", () => ToggleCamera());
            localcamMenu.AddToggleButton($"隱藏相機", ((action) => HideMesh()), () => hideCam);

            localcamMenu.AddSimpleButton("放大", (() => CamScale(1)));
            localcamMenu.AddSimpleButton("縮小", (() => CamScale(-1)));
            localcamMenu.AddSimpleButton("重置", (() => CamScale(0)));

            localcamMenu.AddSimpleButton("Fov+", (() => CamFov(-1)));
            localcamMenu.AddSimpleButton("Fov-", (() => CamFov(1)));
            localcamMenu.AddSimpleButton("重置", (() => CamFov(0)));

            localcamMenu.AddSimpleButton("自拍杆", (() => Stick(true)));
            localcamMenu.AddSimpleButton("朝向自己", (() => RotateHead()));
            localcamMenu.AddToggleButton($"跟隨", ((action) => TrackingSpace()), () => parentTracking.Value);

            localcamMenu.AddToggleButton($"可拿取", ((action) => CamPickup()), () => _pickupable);
        ...
        private void ResetCamera()
        {
            ...
            VRCPlayer player = VRCPlayer.field_Internal_Static_VRCPlayer_0;
            Vector3 pos = GameObject.Find(player.gameObject.name + "/AnimationController/HeadAndHandIK/HeadEffector").transform.position + (player.transform.forward * .25f); // Gets position of Head 
            _localCam.transform.position = pos;
